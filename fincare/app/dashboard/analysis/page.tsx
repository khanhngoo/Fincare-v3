...
import { loadMetricsFromLocalStorage } from "@/lib/metrics-utils"
  ...
  useEffect(() => {
    async function fetchOrGenerateAnalysis() {
      ...
      try {
        ...
        const financialMetrics = loadMetricsFromLocalStorage(applicationId)

        // Validate that we have some financial data
        if (!financialMetrics || (!financialMetrics.total_assets && !financialMetrics.opening_balance)) {
          setError('Please upload financial documents first. Go to Documents page to upload Balance Sheet or Bank Statements.')
          setLoading(false)
          setGenerating(false)
          return
        }

        console.log('Generating analysis with metrics from localStorage:', financialMetrics)

        // Generate analysis with localStorage metrics
        const generateResponse = await fetch('/api/analysis/generate-report', {
          ...
          body: JSON.stringify({
            applicationId,
            loanProductId,
            financialMetrics // Include metrics from localStorage
          }),
        })
